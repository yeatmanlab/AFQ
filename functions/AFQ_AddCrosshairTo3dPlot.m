...
x_y = repmat(coord(2), size(x_x,1), size(x_x,1));
x_z = repmat(coord(3), size(x_x,1), size(x_x,1));
% Y diretion of the crosshair
y_y = [coord(2) - L : coord(2) + L];
y_x = repmat(coord(1), size(x_x,1), size(x_x,1));
y_z = repmat(coord(3), size(x_x,1), size(x_x,1));
% Z direction of the crosshair
z_z = [coord(3) - cL : coord(3) + L];
z_x = repmat(coord(1), size(x_x,1), size(x_x,1));
z_y = repmat(coord(2), size(x_x,1), size(x_x,1));
