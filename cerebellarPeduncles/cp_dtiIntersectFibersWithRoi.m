function [fgOut,contentiousFibers, keep, keepID] = cp_dtiIntersectFibersWithRoi(handles, options, minDist, roi, fg, myFlag)

%% This function creates a new fiber group (fg) that is restricted to fibers that intersectwith an ROI.
%  It is based on the original "dtiIntersectFibersWithRoi" (vistasoft) -- no modifications were made.

%  New fiber group includes only fibers that pass through the specified ROI ("and") or only those that do not ("not"). 
%  Note: If you pass all the other required arguments, you can
%  leave handles empty.
...
%  NOTE! IF THE FIBERS ARE SPARSELY SAMPLED (ie. >1mm step size), THIS WILL NOT WORK WELL.
...
%% History:
%  2004.08.27 - RFD pulled code out of dtiFiberUI and modified the basic algorithm. 
%               I've made it more general purpose and we now use nearpoints to allow a user-specified 'tolerance'.
%  2005.09.26 - RFD removed the unnecessary xform to raw image space. We now do everything in ac-pc space (as we should!).
%  2009.04.25 - ER added the option 'both_points'
%  2016.04.04 - MY added myFlag- should be 'first' or 'last', default 'first'
...
if(~exist('myFlag','var') || isempty(myFlag))
    myFlag = 'first'; % use 'first' as default
end
...
% efficiently in one big array. 
% But now we need to recover the information about which coord belongs to which fiber.
% That's what this confusing bit of code does. Just remember that keepAll
% is a logical array indicating which fiber coords intersect the ROI. 
...
for(ii=1:length(roiCoords))
    ...
    if(endptFlag)
        for(jj=1:length(fg.fibers))
            ...
            if keep(jj,ii)
                keepID(jj,ii) = find(keepAll{ii}(fiberCoord:fiberCoord+1),1,myFlag);
            end
        ...
        for(jj=1:length(fg.fibers))
            ...
            if keep(jj,ii)
                keepID(jj,ii) = find(keepAll{ii}(fiberCoord:fiberCoord+fiberLen-1),1,myFlag);
            end
