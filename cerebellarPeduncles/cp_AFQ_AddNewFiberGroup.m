function afq = cp_AFQ_AddNewFiberGroup(afq,fgName,roi1Name,roi2Name,cleanFibers,computeVals,showFibers,segFgName,overwrite,fgNumber)
...
%  Here, two changes were made to the original "AFQ_AddNewFiberGroup": 
...
%  Original AFQ function adapted by Maya Yablonski - June 2016 
...
%                  subject's ROI directory or a single nifti image in MNI space)
...
if xformRois == 1
    ...
    for ii = runsubs
        ...
        if ~exist(fullfile(sdir,'ROIs',roi1Name),'file') || ...
            ...
            % Load template ROIs in MNI space and transform them into each subject's native space.
            [~, ~, roi1]=dtiCreateRoiFromMniNifti(dt.dataFile, Troi1, invDef, 0);
            [~, ~, roi2]=dtiCreateRoiFromMniNifti(dt.dataFile, Troi2, invDef, 0);
           
            % Save ROIs as .mat files - Maya Yablonski fix 07/21/2016
            % If ROI already exists in subject's directory do not overwrite file.
            if ~exist(fullfile(sdir,'ROIs',roi1Name),'file')
...
%% Segment fiber groups if they do not exist
for ii = runsubs
...
for ii = runsubs
    ...
    if cleanFibers == 1 && (~exist(fgclean_path,'file') || overwrite == 1)
        ...
        if  length(fg_classified.fibers) > 20
            % Clean clipped fiber group if computations are to be
            % done on clipped group
        ...
        % Add fiber group to afq structure
