function [L_fg_vert, R_fg_vert] = babyAFQ_FindVerticalFibers(fgPath,fsROIdir,outdir,thresh, v_crit, minLength)

%%%%%%%%%%%%%%%%%%%%
% This is an infant implementation of AFQ_FindVetticalFibers
% The documentation for the original function in enclosed below

% In brief, the implemented infant-specfic changes are:
% 1) Vertical Fiber ROIs are in volume space, as cortical surfaces are hard to create
...
roi_names = {'VOF_gmwmi_L.mat' 'VOF_gmwmi_R.mat'}; %'middletemporal.mat'
% Path to ROIs
...
R_roi_all = dtiNewRoi('RVOT','r');
    ...
    L_roi = dtiReadRoi(fullfile(fsROIdir,'VOF_gmwmi_L.mat'));
    R_roi = dtiReadRoi(fullfile(fsROIdir,'VOF_gmwmi_R.mat'));
    L_roi_all.coords = vertcat(L_roi_all.coords,L_roi.coords);
    R_roi_all.coords = vertcat(R_roi_all.coords,R_roi.coords);
...
if length(L_fg_vert.fibers)>10
    dtiWriteFiberGroup(L_fg_vert,fullfile(outdir,'Left_VerticalFG_babyAFQ')); L = 1;
else
    L = 0;
end
if length(R_fg_vert.fibers)>10
    dtiWriteFiberGroup(R_fg_vert,fullfile(outdir,'Right_VerticalFG_babyAFQ')); R = 1;
else
    R = 0;
end
