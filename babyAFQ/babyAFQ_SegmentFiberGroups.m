function [fg_classified,fg_unclassified,classification,fg] = ...
    babyAFQ_SegmentFiberGroups(dt6File, fg, Atlas, ...
    useRoiBasedApproach, useInterhemisphericSplit, antsInvWarp, anatFile)
        ...
        if roiID <3 || roiID==5 || roiID==6 || roiID==11 || roiID==12 || roiID==17 || roiID==18 || roiID==19 || roiID==20
            ...
            if recomputeROIs
                ...
                if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                    ...
                    roi3(roiID) = ANTS_CreateRoiFromMniNifti(ROI_img_file, antsInvWarp, [], outfile);
                else
                    [RoiFileName, invDef, roi3(roiID)]=dtiCreateRoiFromMniNifti(dt6File, ROI_img_file, invDef, true);
                end
            else
                RoiFileName=fullfile(fileparts(dt6File), 'ROIs',  [prefix(prefix(ROI_img_file, 'short'), 'short') '.mat']);
