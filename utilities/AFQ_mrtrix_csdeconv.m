function [status, results] = AFQ_mrtrix_csdeconv(dwi_file, response_file, lmax, ...
...
if notDefined('mask_file')
    cmd_str = sprintf('%s %s %s -lmax %d %s -grad %s',...
                       funcName, dwi_file, response_file, lmax, out_file, grad_file);

else 
    cmd_str = sprintf('%s %s %s -lmax %d -mask %s %s -grad %s',...
                       funcName, dwi_file, response_file, lmax, mask_file, out_file, grad_file);
end 
