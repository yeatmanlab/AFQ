function afq = SLF123_AFQ_AddNewFiberGroup3ROI(afq,fgName,roi1Name,roi2Name,roi3Name,roi3type,cleanFibers,computeVals,showFibers,segFgName,overwrite,fgNumber)
% This function is based on AFQ_AddNewFiberGroup

% October 2021 - Adapted by Romi Sagi for segementation of the SLF-I-II-III
...
% afq = AFQ_AddNewFiberGroup(afq, fgName, roi1Name, roi2Name, roi3Name,roi3type, [cleanFibers = true], ...
    ...
    for ii = runsubs
        ...
        if ~exist(fullfile(sdir,'ROIs',roi1Name),'file') || ...
            ...
            if isempty(sn) || isempty(invDef)
                %[sn, Vtemplate, invDef] =
                %mrAnatComputeSpmSpatialNorm(dt.b0, dt.xformToAcpc,
                %template); % Maya following Jason Template branch
...
for ii = runsubs
    ...
    if ~exist(AFQ_get(afq,[prefix(fgName) 'path'],ii),'file') || overwrite == 1
        ...
        fg_classified = dtiIntersectFibersWithRoi([],roi3type, [], roi3, fg_classified);
